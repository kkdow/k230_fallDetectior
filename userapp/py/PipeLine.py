...
class PipeLine:
    ...
    # PipeLine初始化函数
    def create(self,sensor=None,sensor_id=None,hmirror=None,vflip=None,fps=30,to_ide=True,crop_vertical=False):
        with ScopedTiming("init PipeLine",self.debug_mode > 0):
            ...
            if crop_vertical:
                ...
                self.sensor.set_framesize(w = self.display_size[0], h = self.display_size[1], chn=CAM_CHN_ID_1, crop=(crop_x,crop_y,crop_w,crop_h), alignment=12)
                ...
                self.sensor.set_framesize(w = self.display_size[0], h = self.display_size[1], chn=CAM_CHN_ID_1, alignment=12) # 必须加 alignment=12
                self.sensor.set_pixformat(Sensor.YUV420SP, chn=CAM_CHN_ID_1)
                # 通道2给到AI做算法处理，格式为RGB888
